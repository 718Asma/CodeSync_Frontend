            ...
            {data && (
                <div style={{ boxShadow: '5px 0px 10px rgba(0, 0, 0, 0.1)', width: "300px" }} className=" bg-white rounded-lg p-4 mb-4">
                    ...
                    <div className="flex justify-between flex-wrap justify-start">
                        {data.data.friends.map((friend: any, index: number) => (
                            <Link to={`/user/profile/${friend._id}`} style={{width:"100px"}} key={index} className=" cursor-pointer mx-4 hover:text-secondary ">
                                <img src={`http://localhost:3000/${friend.profileImage}`} style={{height:"100px",width:"100px",objectFit: "cover" }}  className="bg-white rounded-lg"/>
